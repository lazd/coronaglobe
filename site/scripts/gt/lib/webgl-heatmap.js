...
WebGLHeatmap = (function() {
  function WebGLHeatmap(options) {
    var error, quad;
    if (options) {
      this.canvas = options.canvas;
      this.width = options.width;
      this.height = options.height;
      this.intensityToAlpha = options.intensityToAlpha;
      this.gradientTexture = options.gradientTexture;
    ...
    this.setGradientTexture(this.gradientTexture);
  ...
  WebGLHeatmap.prototype.setGradientTexture = function(gradientTexture) {
    var output, textureGradient, getColorFun, image;
    ...
    if (gradientTexture) {
      ...
      if (typeof gradientTexture === 'string') {
        ...
        if (gradientTexture.width > 0 && gradientTexture.height > 0) {
          ...
          gradientTexture.addEventListener('load', function() {
      ...
      textureGradient = this.gradientTexture = null;
      getColorFun = 'vec3 getColor(float intensity){\n    vec3 blue = vec3(0.0, 0.0, 1.0);\n    vec3 cyan = vec3(0.0, 1.0, 1.0);\n    vec3 green = vec3(0.0, 1.0, 0.0);\n    vec3 yellow = vec3(1.0, 1.0, 0.0);\n    vec3 red = vec3(1.0, 0.0, 0.0);\n\n    vec3 color = (\n        fade(-0.25, 0.25, intensity)*blue +\n        fade(0.0, 0.5, intensity)*cyan +\n        fade(0.25, 0.75, intensity)*green +\n        fade(0.5, 1.0, intensity)*yellow +\n        smoothstep(0.75, 1.0, intensity)*red\n    );\n    return color;\n}';
    }
    if (this.intensityToAlpha == null) {
      this.intensityToAlpha = true;
    }
    if (this.intensityToAlpha) {
      if (this.alphaRange == null) {
        this.alphaRange = [0, 1];
      }
      var alphaStart = this.alphaRange[0];
      var alphaEnd = this.alphaRange[1];
